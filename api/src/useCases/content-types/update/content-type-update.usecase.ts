...
class ContentTypeUpdateUsecase {
  ...
  async execute(
    payload: IContentTypeUpdateRequestDTO,
  ): Promise<IContentTypeUpdateResponseDTO | null> {
    ...
    if (payload.fieldIds) {
      _contentType.fields = await this.contentFieldRepository.findByIds(
        payload.fieldIds,
      )
      console.log("_contentType.fields", _contentType.fields)
    }
    const contentType = await this.contentTypeRepository.update(
      payload.id,
      _contentType,
    )
    return { contentType }
